function [strain,figData] = eulerLagrange_forfigures(frot, th,ph ,par )
...
%     sigmoid = (sigd.*(2*pi*par.flapFrequency*t).^sign) ./ (sigc+sigd.*(2*pi*par.flapFrequency*t).^sign);
    sigmoid = (tanh(2*pi*par.flapFrequency*(t-0.25) /10) + 1 )/2;
...
% Specify local flapping function
    phi = deg2rad(flapamp) ...
        ...
        + par.harmonic*sin(2*pi*2*par.flapFrequency*t)  ...
        + phi_disturbance ) .* sigmoid  ;
    theta   = 0;
    gamma   = 0;
...
% Specify global rotation function
    ...
    globalAngle_dot3 = (rot_vec(3)*frot ) .*sigmoid +  theta_dot_disturbance;
