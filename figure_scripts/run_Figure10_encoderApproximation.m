...
subplot(221)
    ...
    xlabel('Time from Spike (msec)'); ylabel('Strain (normalized)')
    set(gca,axisOptsSTA{:})
    grid on
    title('\textbf{Experiment}')
subplot(222)
    plot(timeMS,func(timeMS)*1.2 ,':k')
    xlabel('Time from Spike (msec)'); %ylabel('displacement [mm]')
    grid on
...
subplot(223)
    plot(NLDstruct.Bin_Centers_Store{:},NLDstruct.fire_rate{:}/max(NLDstruct.fire_rate{:}),'k')
     xlabel('Feature Projection, $\xi$');ylabel('Prob. of Firing')
    set(gca, axisOptsNLD{:})
    grid on
subplot(224)
    plot(s,funNLD(s),':k')
...
%% 
print(fig04, ['figs' filesep 'Figure10_encoderVariation' ], '-dpng', '-r300');
...
print(fig04, ['figs' filesep 'Figure10_encoderVariation' ], '-dsvg');
