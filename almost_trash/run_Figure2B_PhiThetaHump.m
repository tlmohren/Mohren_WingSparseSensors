...
total_humpsim = ind_SSPOCon(17:end-2);
total_humpsimoff = ind_SSPOCoff(17:end-2);
ind_SSPOCon_phi = ind_SSPOCon(17: 17+ length(total_humpsim)/2 -1)
ind_SSPOCon_theta = ind_SSPOCon(17+ length(total_humpsim)/2 :end-2)
ind_SSPOCoff_phi = ind_SSPOCoff(17: 17+ length(total_humpsimoff)/2 -1)
ind_SSPOCoff_theta = ind_SSPOCoff(17+ length(total_humpsimoff)/2 :end-2)

col = {'-k','-r'};
dotcol = {'.k','.r'}; 


Datamat(:,[1:10,20:40],:) = 0;
...
%      title('$\phi$-disturbance
     ...
     ylabel('accuracy')
    axis( [ min(par.phi_distList), max(par.phi_distList),0.4,1])
