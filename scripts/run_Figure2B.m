...
% ind_SSPOCoff = [ 75:2:115, 127: 2:167];

ind_SSPOCoff = [ 75:2:158];
...
if fig2B_on == 1
    ...
    for j = 1:n_y
        ...
        %         a = shadedErrorBar(realNumbers, meanVec(realNumbers),stdVec(realNumbers),col{2},0.9);
            ...
            meanval = mean( nonzeros(  dataStructAll.dataMatTot(sub_nr+64,:)  ) );
            stdval = std( nonzeros(    dataStructAll.dataMatTot(sub_nr+64,:)  ) );
...
% % % %             %--------------------------------Allsensors no NF-------------------------    
% % % %             meanval = mean( nonzeros(  dataStructAll.dataMatTot(sub_nr+64+21,:)  ) );
% % % %             stdval = std( nonzeros(    dataStructAll.dataMatTot(sub_nr+64+21,:)  ) );
% % % % 
% % % % 
% % % % %             err_loc = 37;
...
%      plot2svg(['figs' filesep 'Figure2B_' par.saveNameParameters] , fig2B ) 
end
...
%     fig1A = figure();
    ...
    fig2B=figure('Position', [500, 100, 500, 400]);
    sub_nr = 10;
    %         subplot(n_y,n_x, sub_nr)
    hold on
